 ...
 * Period Extractor
 * 
 * Extraherar år och månad från Facebook API CSV-filnamn
 * Hanterar format: FB_YYYY_MM.csv (t.ex. FB_2025_08.csv)
 */

/**
 * Regex för att matcha Facebook API filnamnsformat
 * Format: FB_YYYY_MM.csv
 */
const FB_FILENAME_PATTERN = /^FB_(\d{4})_(\d{1,2})\.csv$/i;

/**
 * Alternativt regex för flexibilitet (med eller utan FB_ prefix)
 * Format: [FB_]YYYY_MM.csv
 */
const FLEXIBLE_FILENAME_PATTERN = /^(?:FB_)?(\d{4})_(\d{1,2})\.csv$/i;

/**
 * Extraherar period från Facebook API filnamn
 * @param {string} filename - Filnamn att analysera
 * @param {boolean} strict - Om true, kräv exakt FB_YYYY_MM.csv format
...
export function extractPeriodFromFilename(filename, strict = true) {
  ...
  const pattern = strict ? FB_FILENAME_PATTERN : FLEXIBLE_FILENAME_PATTERN;
...
/**
 * Validerar om ett år är rimligt för Facebook API data
 * @param {number} year - År att validera
 * @returns {boolean} - True om året är giltigt
 */
export function isValidYear(year) {
  const currentYear = new Date().getFullYear();
  // Facebook grundades 2004, tillåt data från 2005 till 5 år framåt
  return year >= 2005 && year <= currentYear + 5;
}
 ...
 * @param {number} year - År
 * @param {number} month - Månad
 * @returns {string} - Standardiserat filnamn (FB_YYYY_MM.csv)
 */
export function createStandardFilename(year, month) {
  ...
  return `FB_${year}_${paddedMonth}.csv`;
}
 ...
 * @returns {string} - Formaterad period (t.ex. "Augusti 2025")
 */
