 ...
 * Period Validator
 * 
 * Validerar FB_YYYY_MM.csv filformat och CSV-innehåll för Facebook API data
...
function validateFilenameAndExtractPeriod(file) {
  ...
  if (!period) {
    errors.push({
      type: ERROR_TYPES.FILENAME,
      message: `Filnamnet följer inte formatet FB_YYYY_MM.csv (t.ex. FB_2025_08.csv)`,
      severity: 'error'
    });
...
function validateDataRow(row, rowNumber) {
  ...
  if (!row.Page || row.Page.trim() === '') {
    errors.push({
      type: ERROR_TYPES.DATA_CONTENT,
      message: `Rad ${rowNumber}: Saknar Page (sidnamn)`,
      severity: 'error'
    });
  }
  ...
  if (!row['Page ID'] || row['Page ID'].toString().trim() === '') {
    errors.push({
      type: ERROR_TYPES.DATA_CONTENT,
      message: `Rad ${rowNumber}: Saknar Page ID`,
      severity: 'error'
    });
  }
